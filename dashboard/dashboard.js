  ...
  try {
    ...
    if (data.error) {
      return Swal.fire({
        icon: "error",
        title: "خطا",
        text: data.error || "خطا در دریافت اطلاعات",
        confirmButtonText: "باشه",
        customClass: {
          popup: "swal2-rtl swal2-glass",
          confirmButton: "btn btn-primary",
    ...
    if (!courses.length) {
      return Swal.fire({
        icon: "info",
        title: "اطلاعات",
        text: "هیچ درسی برای این جلسه وجود ندارد",
        confirmButtonText: "باشه",
        customClass: {
          popup: "swal2-rtl swal2-glass",
          confirmButton: "btn btn-primary",
    ...
    courses.sort((a, b) => {
      if ((a.exam_type || "") === (b.exam_type || ""))
        return (Number(a.course_code) || 0) - (Number(b.course_code) || 0);
    ...
    try {
      ...
      const persianToLatin = (s) =>
        String(s || "").replace(
          /[۰-۹]/g,
          (d) => "0123456789"["۰۱۲۳۴۵۶۷۸۹".indexOf(d)]
        );
      const partsDate = (examDate || "")
        .split("/")
        .map((s) => persianToLatin(s).replace(/[^0-9]/g, ""));
    ...
    const persAcad = `<span dir="ltr" style="unicode-bidi: embed; direction: ltr;">${toPersianDigits(
      acadLeft
    )}-${toPersianDigits(acadRight)}</span>`;

    // Prepare printable HTML (portrait A4) with 20 courses per page
    const { headMarkup: fontHeadMarkup } = getVazirFontMeta();

    let html = `<!doctype html><html lang="fa" dir="rtl"><head><meta charset="utf-8"><title>صورتجلسه آزمون</title>${fontHeadMarkup}`;
    html += `<style>
    ...
    let university = esc(
      cfg.University || document.getElementById("footerText")?.textContent || ""
    );
    ...
    const headerHtml = `
                        ...
                        <div class="meta">آزمون دروس زیر در ${semesterLabel} سالتحصیلی ${persAcad} با حضور امضاء کنندگان زیر در ساعت ${toPersianDigits(
      examTime
    )} مورخ ${toPersianDigits(
      examDate
    )} شروع گردید. (نمونه سوال ضمیمه می باشد)</div>`;

    function buildTable(slice, startIndex) {
      // table header
      let t =
        '<table class="courses"><thead><tr>' +
    ...
    for (let p = 0; p < pages; p++) {
      ...
      const bossLabel = `نام و نام خانوادگی رئیس مرکز/ معاون مرکز/ سرپرست واحد: ${
        bossName || "________________"
      }`;
      html += `<div class="sign"><div class="main">${bossLabel}</div><div class="sig">امضاء</div></div>`;

      // Row 2: مسئول آموزش
      const headLabel = `نام و نام خانوادگی رئیس اداره آموزش: ${
        headName || "________________"
      }`;
      html += `<div class="sign"><div class="main">${headLabel}</div><div class="sig">امضاء</div></div>`;

      // Row 3: مسئول جلسه
      const chairLabel = `نام و نام خانوادگی مسئول جلسه: ${
        chairName || "________________"
      }`;
      ...
      html += `<div class="page-footer-bar">صفحه ${toPersianDigits(
        p + 1
      )} از ${toPersianDigits(pages)}</div>`;
    ...
    Swal.fire({
      icon: "error",
      title: "خطا",
      text: "خطا در آماده‌سازی صورتجلسه",
      confirmButtonText: "باشه",
      customClass: {
        popup: "swal2-rtl swal2-glass",
        confirmButton: "btn btn-primary",
...
try {
  ...
  if (infoBtn) {
    infoBtn.addEventListener("click", async () => {
      ...
      try {
        const response = await guardedFetch("../API/getStatistics.php", {
          cache: "no-store",
        });
        ...
        insightDefinitions.forEach((def) => {
          ...
          const unitText =
            def.unit || (def.category === "session" ? "نفر" : "");
          ...
          const displayLine2 = line2Parts.length
            ...
            : "بدون تاریخ";
          const displayCourseCode = entry.course_code
            ? toPersianDigits(entry.course_code)
            : "";
          ...
          const classes = ["session-mini-card", "insight-card", def.variant]
            .filter(Boolean)
            .join(" ");
          const attributes = [
          ...
          if (rawDate)
            attributes.push(`data-exam-date="${escapeAttr(rawDate)}"`);
          if (rawTime)
            attributes.push(`data-exam-time="${escapeAttr(rawTime)}"`);
          if (entry.course_code)
            attributes.push(
              `data-course-code="${escapeAttr(entry.course_code)}"`
            );
          if (entry.course_name)
            attributes.push(
              `data-course-name="${escapeAttr(entry.course_name)}"`
            );
          if (entry.course_type)
            attributes.push(
              `data-course-type="${escapeAttr(entry.course_type)}"`
            );
          if (Array.isArray(entry.matches) && entry.matches.length) {
            try {
              attributes.push(
                `data-matches="${escapeAttr(JSON.stringify(entry.matches))}"`
              );
            } catch (jsonErr) {
              console.warn(
                "Failed to encode matches for insight",
                def.key,
                jsonErr
              );
            }
        ...
        const cardsHtml = `<div class="session-mini-grid insight-grid">${cards.join(
          ""
        )}</div>`;
        ...
        function openTieModal(labelText, labelBase, matches) {
          ...
          const parseDateParts = (rawDate) => {
            ...
            const normalized = toEnglishDigits(String(rawDate || "")).replace(
              /-/g,
              "/"
            );
          ...
          const parseTimeParts = (rawTime) => {
            ...
            const normalized = toEnglishDigits(String(rawTime || "")).replace(
              /[^0-9:]/g,
              ""
            );
            const parts = normalized
          ...
          const sortedMatches = Array.isArray(matches)
            ...
            : [];
          const cardsMarkup = sortedMatches
            .map((match) => {
              ...
              return `
                            <div class="session-mini-card insight-card insight-subcard" data-exam-time="${escapeAttr(
                              mTime
                            )}" data-exam-date="${escapeAttr(mDate)}">
        ...
        function openInsightsModal() {
          return Swal.fire({
            ...
            didOpen: () => {
              ...
              if (!container) return;
              const cardNodes = container.querySelectorAll(
              ...
              cardNodes.forEach((cardEl) => {
                ...
                const labelBase =
                  cardEl.getAttribute("data-label-base") || label;
                const displayLine2 =
                  cardEl.getAttribute("data-display-line2") || "";
                ...
                const tieCount = Number(
                  cardEl.getAttribute("data-tie-count") || 0
                );
                ...
                if (courseName) {
                  const tooltip = courseCode
                    ? `درس ${courseName} (کد ${courseCode})`
                    : `درس ${courseName}`;
                    ...
                    if (type === "session" && tieCount > 1 && matches.length) {
                      ...
                      setTimeout(() => {
                        applyNextExamOverride(examDate, examTime, {
                          customTitle: `${label} (${displayLine2})`,
                        });
                        showNextExamReport();
                      }, 150);
                      ...
                      Swal.close();
                      setTimeout(() => {
                        loadCourseReportByCode(courseCode, {
                          showErrors: true,
                        });
                      }, 150);
                    }
...
async function printSeatNumbersReport() {
  try {
    ...
    const ctxDate =
      context && context.exam_date ? normalizeDate(context.exam_date) : null;
    const ctxTime =
      context && context.exam_time ? normalizeTime(context.exam_time) : null;
    ...
    if (needsFetch && ctxDate && ctxTime) {
      try {
        ...
        return Swal.fire({
          icon: "error",
          title: "خطا",
          text: fetchErr.message || "خطا در دریافت اطلاعات جلسه",
          confirmButtonText: "باشه",
          customClass: {
            popup: "swal2-rtl swal2-glass",
            confirmButton: "btn btn-primary",
    ...
    if (!students.length) {
      return Swal.fire({
        icon: "info",
        title: "اطلاعات",
        text: "هیچ دانشجویی برای چاپ یافت نشد",
        confirmButtonText: "باشه",
        customClass: {
          popup: "swal2-rtl swal2-glass",
          confirmButton: "btn btn-primary",
    ...
    const repDateRaw =
      report && report.exam_date
        ? report.exam_date
        : context && context.exam_date
        ? context.exam_date
        : "";
    const repTimeRaw =
      report && report.exam_time
        ? report.exam_time
        : context && context.exam_time
        ? context.exam_time
        : "";
    ...
    const normalize = (s) => ({
      ...
      seat_number:
        typeof s.seat_number !== "undefined" ? String(s.seat_number) : "",
    ...
    entries.sort((a, b) => {
      if (groupByCourse) {
        ...
        if (courseA !== courseB)
          return (
            courseA.localeCompare(courseB, "fa") ||
      ...
      if (lnA !== lnB)
        return lnA.localeCompare(lnB, "fa") || lnA.localeCompare(lnB);
    ...
    const { headMarkup: fontHeadMarkup } = getVazirFontMeta();
    let docHtml = `<!doctype html><html lang="fa" dir="rtl"><head><meta charset="utf-8"><title>گزارش شماره صندلی</title>${fontHeadMarkup}`;
    docHtml += `<style>
    ...
    for (let p = 0; p < totalPages; p++) {
      ...
      docHtml += `<div class="report-header"><div class="report-title">${esc(
        university
      )}</div>`;
      ...
      const prStart =
        typeof toPersianDigits === "function"
          ? toPersianDigits(pageStart)
          : String(pageStart);
      const prEnd =
        typeof toPersianDigits === "function"
          ? toPersianDigits(pageEnd)
          : String(pageEnd);
      ...
      if (groupKeys.length) {
        ...
        rows.sort(
          (a, b) =>
            ...
            a.class_name.localeCompare(b.class_name, "fa")
        );
        ...
        mapHtml += `<div class="report-meta" style="font-size:14pt;margin-top:6px;font-weight:700;">${esc(
          title
        )}</div></div>`;
        ...
        rows.forEach((r) => {
          ...
          mapHtml += `<tr style="text-align:center;"><td style="padding:8px;border:1px solid #ddd;">${from}</td><td style="padding:8px;border:1px solid #ddd;">${to}</td><td style="padding:8px;border:1px solid #ddd;">${cnt}</td><td style="padding:8px;border:1px solid #ddd;">${esc(
            r.building
    ...
    function buildSinglePageHtml(fontPt) {
      let h = `<!doctype html><html lang="fa" dir="rtl"><head><meta charset="utf-8"><title>پیش‌نمایش چاپ</title>${fontHeadMarkup}`;
      h += `<style>
                @page { size: A4 landscape; margin: 4mm; }
                html,body { margin:0; padding:0; }
                ...
                .col-wrap { display:flex; gap:6px; position:relative; }
                .col { width:50%; }
                .page-range { text-align: center; color: #666; font-size: ${Math.max(
                  8,
                  fontPt
                )}pt; margin-top: 4px; }
            </style></head><body>`;
      ...
      [left, right].forEach((colArr, ci) => {
        ...
        colArr.forEach((r, idx) => {
          ...
          h += `<tr><td title="${fullName}">${fullName}</td><td title="${courseName}">${courseName}</td><td class="seat-col">${
            r.seat_number || ""
          }</td></tr>`;
        });
        h += `</tbody></table></div>`;
      ...
      const pStartFa =
        typeof toPersianDigits === "function"
          ? toPersianDigits(pStart)
          : String(pStart);
      const pEndFa =
        typeof toPersianDigits === "function"
          ? toPersianDigits(pEnd)
          : String(pEnd);
    ...
    (async () => {
      ...
      if (fits) {
        try {
          ...
          let finalHtml = docHtml.replace(
            /font-size:\s*[\d.]+pt;/g,
            `font-size: ${testFontPt}pt;`
          );

          finalHtml = finalHtml.replace(
            "</head>",
            `<style>.report-meta{font-size:20pt !important; font-weight:900 !important;} .kroki-page .kroki-title-large{font-size: 32pt !important; font-weight:900 !important;}</style></head>`
          );
          fullDoc.open();
        ...
        let fallbackHtml = `<!doctype html><html lang="fa" dir="rtl"><head><meta charset="utf-8"><title>گزارش شماره صندلی</title>${fontHeadMarkup}`;
        ...
        for (let p = 0; p < pages; p++) {
          ...
          slice.forEach((r, i) => {
            fallbackHtml += `<tr><td>${startIdx + i + 1}</td><td>${esc(
              (r.last_name || "") + " " + (r.first_name || "")
            )}</td><td>${esc(
              r.course_name || ""
            )}</td><td class="seat-col">${esc(r.seat_number || "")}</td></tr>`;
          });
        ...
        if (!w) {
          return Swal.fire({
            icon: "error",
            title: "خطا",
            text: "لطفاً باز شدن پنجره جدید را در مرورگر فعال کنید",
            confirmButtonText: "باشه",
            customClass: {
              popup: "swal2-rtl swal2-glass",
              confirmButton: "btn btn-primary",
        ...
        setTimeout(() => {
          try {
            ...
            console.error("Print error (fallback window):", e);
          }
        }, 450);
      }
    })();
    ...
    console.error(
      "Error building printable report (printEssentialsSecretary):",
      err
    );
    const msg =
      err && (err.message || err.toString())
        ? String(err.message || err)
        : "خطا در آماده‌سازی گزارش چاپ";
    const stack =
      err && err.stack
    ...
    Swal.fire({
      icon: "error",
      title: "خطا",
      html: `<div style="text-align:right">${esc(
        msg
      )}<br/><pre style="text-align:left;white-space:pre-wrap;font-size:0.8rem;color:#666">${esc(
        stack
      )}</pre></div>`,
      confirmButtonText: "باشه",
      customClass: {
...
async function printProctorNotices() {
  try {
    ...
    try {
      const response = await guardedFetch(
        "../API/getProctorNotifications.php",
        { cache: "no-store" }
      );
    ...
    if (!proctors.length) {
      ...
      return Swal.fire({
        icon: "info",
        title: "اطلاعات",
        text: "برای هیچ مراقبی جلسهٔ ثبت‌شده‌ای وجود ندارد.",
        confirmButtonText: "باشه",
        customClass: {
          popup: "swal2-rtl swal2-glass",
          confirmButton: "btn btn-primary",
    ...
    const normalizeDate = (value) => {
      const ascii = toEnglishDigits(String(value || "")).replace(
        /[^0-9/]/g,
        ""
      );
    ...
    if (!sortedDates.length || !sortedTimes.length) {
      ...
      return Swal.fire({
        icon: "info",
        title: "اطلاعات",
        text: "جدول زمان‌بندی مراقبین خالی است.",
        confirmButtonText: "باشه",
        customClass: {
          popup: "swal2-rtl swal2-glass",
          confirmButton: "btn btn-primary",
    ...
    const persianYearRange =
      acadStart && acadEnd
        ? `<span dir="ltr" style="unicode-bidi: embed; direction: ltr;">${toPersianDigits(
            acadStart
          )}-${toPersianDigits(acadEnd)}</span>`
        : "نامشخص";
    const termPhrase = `${semesterLabel}${
      acadStart ? ` سال‌تحصیلی ${persianYearRange}` : ""
    }`;
    ...
    try {
      ...
      try {
        ...
        console.warn(
          "Config fetch for proctor notices failed, using cached config if any",
          cfgFetchErr
        );
        cfg =
          window.appConfig && typeof window.appConfig === "object"
            ? window.appConfig
            : null;
      }
      if (cfg && cfg.Chairman) {
    ...
    const complianceText =
      "لطفاً در کلیه جلسات امتحانی از همراه داشتن موبایل خودداری کنید. همراه داشتن جزوه یا کتاب جهت مطالعه در سر جلسه ممنوع است. ضمن حفظ سکوت، از صحبت با سایر عوامل و دانشجویان حاضر در جلسه پرهیز نمایید. حضور حداقل یک ربع پیش از شروع جلسه با اتیکت عکس‌دار نصب‌شده الزامی است.";

    const { headMarkup: fontHeadMarkup } = getVazirFontMeta();
    let html = `<!doctype html><html lang="fa" dir="rtl"><head><meta charset="utf-8"><title>ابلاغ مراقبین</title>${fontHeadMarkup}`;
    html += `<style>
    ...
    const renderTable = (dateSlice, sessionSet) => {
      let tableHtml =
        '<table class="schedule-table"><thead><tr><th class="row-col"><span>#</span></th><th class="date-col">تاریخ</th>';
      sortedTimes.forEach((timeObj) => {
        tableHtml += `<th class="time-col"><span>${toPersianDigits(
          timeObj.normalized
        )}</span></th>`;
      });
      ...
      dateSlice.forEach((dateObj) => {
        tableHtml += "<tr>";
        const rowLabel = dateObj.rowNumber
          ? toPersianDigits(dateObj.rowNumber)
          : "";
        tableHtml += `<td>${rowLabel}</td>`;
        tableHtml += `<td class="date">${toPersianDigits(
          dateObj.normalized
        )}</td>`;
        sortedTimes.forEach((timeObj) => {
          ...
          tableHtml += `<td>${
            hasSession ? '<span class="check-mark">&#10003;</span>' : ""
          }</td>`;
        });
        tableHtml += "</tr>";
      });
    ...
    proctors.forEach((item) => {
      ...
      const proctorDates = sortedDates.filter((dateObj) => {
        return sortedTimes.some((timeObj) =>
          sessionSet.has(`${dateObj.normalized}|${timeObj.normalized}`)
        );
      });
      ...
      if (sessionCount > 0 && dayCount > 0) {
        countsText = ` (${toPersianDigits(
          sessionCount
        )} جلسه در ${toPersianDigits(dayCount)} روز)`;
      ...
      const effectiveDatesSource = proctorDates.length
        ? proctorDates
        : sortedDates;
      const effectiveDates = effectiveDatesSource.map((dateObj, idx) => ({
        ...dateObj,
        rowNumber: idx + 1,
      }));
      const useTwoColumns =
        (sortedTimes.length <= 4 && effectiveDates.length > 10) ||
        effectiveDates.length > 14;
      const columns = useTwoColumns ? 2 : 1;
      const rowsPerColumn =
        columns > 1
          ? Math.max(1, Math.ceil(effectiveDates.length / columns))
          : Math.max(1, effectiveDates.length);
      const chunks = [];
      ...
      html += `<div class="compliance-note">${escapeHtml(
        complianceText
      )}</div>`;
    ...
    try {
      ...
      if (cw) {
        ...
        try {
          ...
          console.error("Print error (proctor notices):", e);
        }
        setTimeout(cleanup, 5000);
      } else {
        setTimeout(cleanup, 300);
    ...
    const msg =
      error && error.message
        ? error.message
        : "خطا در آماده‌سازی ابلاغ مراقبین";
    Swal.fire({
      icon: "error",
      title: "خطا",
      text: msg,
...
async function printEssentialsSecretary() {
  try {
    ...
    const university =
      (document.getElementById("footerText")?.textContent || "")
        .trim()
        .replace(/^نسار\s*-\s*/, "") || "گزارش ملزومات منشی جلسه";
    ...
    const normalizedExamDate =
      report && report.exam_date ? normalizeDate(report.exam_date) : "";
    const normalizedExamTime =
      report && report.exam_time ? normalizeTime(report.exam_time) : "";
    ...
    if (normalizedExamDate && normalizedExamTime) {
      try {
        const assignResp = await guardedFetch(
          `../API/getExamAssignments.php?exam_date=${encodeURIComponent(
            normalizedExamDate
          )}&exam_time=${encodeURIComponent(normalizedExamTime)}`,
          { cache: "no-store" }
        );
        if (assignResp.ok) {
          const assignData = await assignResp.json();
          if (assignData && assignData.success) {
            const rawList = Array.isArray(assignData.proctors)
              ? assignData.proctors
              : [];
            proctorAssignments = rawList.filter(
              (p) =>
                p &&
                typeof p.proctor_name === "string" &&
                p.proctor_name.trim().length > 0
            );
        ...
        console.warn(
          "Failed to load proctor assignments for secretary report:",
          assignErr
        );
      }
    }

    const hasCompleteProctorAssignments =
      proctorAssignments.length > 0 &&
      proctorRequiredTotal > 0 &&
      proctorAssignedTotal >= proctorRequiredTotal;
    ...
    // Build HTML
    const { headMarkup: fontHeadMarkup } = getVazirFontMeta();
    let docHtml = `<!doctype html><html lang="fa" dir="rtl"><head><meta charset="utf-8"><title>ملزومات منشی جلسه</title>${fontHeadMarkup}`;
    docHtml += `<style>
    ...
    if (hasCompleteProctorAssignments) {
      ...
      if (indexed.length) {
        ...
        const paperSavingEnabled =
          String(
            (window.appConfig && window.appConfig.PaperSaving) || ""
          ).toUpperCase() === "YES";
        const startRosterOnNewPage =
          rosterNeedsPageBreak || !paperSavingEnabled;
        if (startRosterOnNewPage) {
          proctorSectionOnNewPage = true;
        }
        let html = startRosterOnNewPage
          ? `<div class="page proctor-page">`
          : "";
        html += `<div class="proctor-roster">`;
        html += `<div class="roster-inner">`;
        html += `<div class="proctor-title">لیست مراقبین جلسه</div>`;
        html += `<div class="proctor-meta">${toPersianDigits(
          examTime
        )} &nbsp; | &nbsp; ${toPersianDigits(examDate)}</div>`;
        ...
        columns.forEach((col) => {
          ...
          col.forEach((item) => {
            html += `<tr><td class="index">${toPersianDigits(
              item.index
            )}</td><td class="name">${esc(item.name)}</td></tr>`;
          });
    ...
    docHtml += `<div class="page">`;
    docHtml +=
      `<div class="header" style="background:transparent;color:#000;padding:4px 2px 6px 2px;text-align:center;">` +
      `<div class="title" style="font-size:16pt;font-weight:900;color:#000;margin-bottom:2px;">لیست منشی جلسه</div>` +
      `<div class="meta" style="font-size:12pt;margin-top:2px;color:#000;font-weight:700;">${toPersianDigits(
        examTime
      )} &nbsp; | &nbsp; ${toPersianDigits(examDate)}</div>` +
      `</div>`;
    ...
    for (const exType of examOrder.concat([""])) {
      ...
      const coursesForType = courses.filter((c) => {
        const s = students.find(
          (st) =>
            st.course_code === c.course_code && (st.exam_type || "") === exType
        );
        return !!s;
      });
      ...
      docHtml += `<div class="course-type-group"><div class="etypeBar"><div class="label">${esc(
        exType || "سایر"
      )}</div></div></div>`;
      ...
      for (const course of coursesForType) {
        ...
        courseIndex += 1;
        docHtml +=
          `<table class="course-header"><tr>` +
          `<td class="course-index">${toPersianDigits(courseIndex)}</td>` +
          `<td class="course-info">${toPersianDigits(
        ...
        stu.forEach((s) => {
          ...
          const key = b + "||" + cl;
          if (!groups[key])
            groups[key] = { building: b, class_name: cl, nums: [] };
          try {
        ...
        docHtml +=
          `<table class="nested" style="table-layout:fixed;width:100%;page-break-inside:avoid;min-width:0;"><thead><tr>` +
        ...
        if (!gkeys.length) {
          ...
          rows.forEach((r) => {
            docHtml +=
              `<tr>` +
              `<td style="text-align:center;padding:3px 6px;font-size:11pt;">${toPersianDigits(
                r.start
              )}</td>` +
              `<td style="text-align:center;padding:3px 6px;font-size:11pt;">${toPersianDigits(
                r.end
              ...
              `<td style="padding:3px 6px;font-size:10pt;text-align:right;">${toPersianDigits(
                esc(r.building)
              )}</td>` +
              `<td style="padding:3px 6px;font-size:10pt;text-align:right;">${toPersianDigits(
                esc(r.class_name)
    ...
    const remainingCourses = courses.filter(
      (c) => !usedCourses.has(c.course_code)
    );
    if (remainingCourses.length) {
      ...
      remainingCourses.forEach((course) => {
        ...
        docHtml += `<div style="margin-bottom:6px;"><strong>${codeStr}</strong> — ${nameStr} &nbsp; <span class="small">(تعداد: ${toPersianDigits(
          total
        )})</span></div>`;
      });
      docHtml += `</div>`;
    ...
    try {
      console.error(
        "Error building printable report (detailed):",
    ...
    const msg =
      err && err.message ? String(err.message) : "خطا در آماده‌سازی گزارش چاپ";
    const stack =
      err && err.stack
        ? String(err.stack).split("\n").slice(0, 6).join("\n")
        : "";
    ...
    Swal.fire({
      icon: "error",
      title: "خطا در آماده‌سازی گزارش چاپ",
      html: `<div style="text-align:right;direction:ltr;white-space:pre-wrap;">${htmlEsc(
        msg
      )}<br><small style='color:#666;margin-top:8px;display:block;'>${htmlEsc(
        stack
      )}</small></div>`,
      confirmButtonText: "باشه",
      customClass: {
...
async function printEssentialsReproduction() {
  try {
    ...
    const university =
      (document.getElementById("footerText")?.textContent || "")
        .trim()
        .replace(/^نسار\s*-\s*/, "") || "گزارش ملزومات اتاق تکثیر";
    ...
    const { headMarkup: fontHeadMarkup } = getVazirFontMeta();
    let docHtml = `<!doctype html><html lang="fa" dir="rtl"><head><meta charset="utf-8"><title>ملزومات اتاق تکثیر</title>${fontHeadMarkup}`;
    docHtml += `<style>
            @page { size: A4 portrait; margin: 6mm; }
            html, body { margin: 0; padding: 0; }
            body { font-family: Vazir, Tahoma, Arial, sans-serif; color: #111; font-size: 11pt; }
    ...
    const electronicCourses = courses.filter((c) =>
      students.some(
        (st) =>
          st.course_code === c.course_code &&
          (st.exam_type || "") === "الکترونیکی"
      )
    );
    if (electronicCourses.length) {
      ...
      electronicCourses.forEach((course) => {
        ...
        if (cnt > 0) {
          ...
          docHtml +=
            `<tr>` +
            `<td style=\"text-align:center;font-weight:700;\">${toPersianDigits(
              i
            )}</td>` +
      ...
      if (i === 0)
        docHtml += `<tr><td colspan=\"4\" style=\"text-align:center\">بدون درس الکترونیکی</td></tr>`;
      docHtml += `</tbody></table>`;
      docHtml += `</div></div>`;
    }

    // 2) Written section: full detail like secretary
    const writtenCourses = courses.filter((c) =>
      students.some(
        (st) =>
          st.course_code === c.course_code && (st.exam_type || "") === "کتبی"
      )
    );
...
async function printEssentialsDescriptive() {
  try {
    ...
    const university =
      (document.getElementById("footerText")?.textContent || "")
        .trim()
        .replace(/^نسار\s*-\s*/, "") || "برچسب پاکت‌های تشریحی";
    ...
    const ctxTime =
      context && context.exam_time ? normalizeTime(context.exam_time) : null;
    if (
      (!report || !report.exam_date || !report.exam_time) &&
      ctxDate &&
      ctxTime
    ) {
      try {
    ...
    const commonHead = `<!doctype html><html lang="fa" dir="rtl"><head><meta charset="utf-8"><title>برچسب پاکت‌های تشریحی</title>${fontHeadMarkup}`;
    ...
    const finalHtml =
      commonHead + commonStyle + `<body>` + pages.join("") + `</body></html>`;
