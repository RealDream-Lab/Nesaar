...
try {
    ...
    foreach ($futureExams as $exam) {
        ...
        if (isset($typeCounts[$d][$t])) {
            foreach ($typeCounts[$d][$t] as $k => $v) {
                if ($k === 'الکترونیکی' || mb_stripos($k, 'الکت') !== false)
                    $etypeCounts['الکترونیکی'] += (int)$v;
                elseif ($k === 'کتبی' || mb_stripos($k, 'کتب') !== false)
    ...
    foreach ($futureExamsOutput as $fe) {
        if (!empty($fe['exam_type_counts'])) {
            foreach ($fe['exam_type_counts'] as $k => $v) {
                if ($k === 'الکترونیکی' || mb_stripos($k, 'الکت') !== false)
                    $futureExamTypeTotals['الکترونیکی'] += (int)$v;
                else
                    $futureExamTypeTotals['کتبی'] += (int)$v;
            }
        }
    }
    ...
    foreach ($typeCounts as $d => $times) {
        foreach ($times as $t => $types) {
            ...
            // electronic-only aggregation removed (card deleted)
        }
    }
    ...
    // maxElectronic removed: no tie/match aggregation needed
