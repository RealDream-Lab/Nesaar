...
try {
    ...
    } elseif ($reportType === 'multi_exam_students') {
        generateMultiExamStudentsReport($pdo, $mpdf, $examDate, $examTime, $config);
    } else {
...
/**
 * Generate report for multi-exam students (same format as reproduction report)
 */
function generateMultiExamStudentsReport($pdo, $mpdf, $examDate, $examTime, $config)
{
    ...
    if (!$multiExamModeEnabled) {
        $mpdf->AddPage('P');
        $mpdf->WriteHTML('<p style="text-align: center; color: red; font-size: 14pt; font-weight: bold;">حالت چند آزمونی فعال نیست</p>');
        return;
    }
    ...
    if (empty($courses)) {
        $mpdf->AddPage('P');
        $mpdf->WriteHTML('<p style="text-align: center; color: red; font-size: 14pt; font-weight: bold;">هیچ درسی برای این جلسه یافت نشد</p>');
        return;
    }
    ...
    // If no multi-exam students, show message
    if (empty($multiExamStudents)) {
        $mpdf->AddPage('P');
        $mpdf->WriteHTML('<p style="text-align: center; color: #666; font-size: 14pt;">هیچ دانشجوی چند آزمونی برای این جلسه وجود ندارد</p>');
        return;
    }
    ...
    // Group multi-exam students by their primary seat location
    $locationMultiExam = [];
    foreach ($multiExamStudents as $sid => $exams) {
        // Find primary seat (minimum seat number)
        $seatNumbers = [];
    ...
    <div class="header">
        <div class="title">دانشجویان چند آزمونی</div>
        <div class="meta">' . toPersianDigits($examTime) . ' | ' . toPersianDigits($examDate) . '</div>
    </div>
    <div style="background: #dc3545; color: #fff; font-size: 12pt; font-weight: bold; text-align: center; padding: 6px; border-radius: 8px; margin: 10px 0;">
        لیست دانشجویان چند آزمونی بر اساس مکان استقرار (' . toPersianDigits(count($multiExamStudents)) . ' نفر)
    </div>
    ';
    ...
    foreach ($locationMultiExam as $key => $loc) {
        ...
        $html .= '<div class="location-multi-box">';
        $html .= '<table class="location-multi-header"><tr>
            <td class="location-index-cell"><div class="location-index-box">' . toPersianDigits($locationIndex) . '</div></td>
            <td class="location-info">' . $loc['building'] . ' | ' . $loc['class_name'] . '</td>
            <td style="text-align: left; font-weight: bold; font-size: 10pt;">' . toPersianDigits(count($loc['students'])) . ' نفر</td>
        </tr></table>';
        ...
        $html .= '<table class="multi-exam-table"><thead><tr>
            ...
            <th style="width: 20%;">نام درس</th>
            <th style="width: 8%;">صندلی</th>
            <th style="width: 18%;">مکان صندلی</th>
            <th style="width: 25%;">ارسال سوالات به</th>
        </tr></thead><tbody>';
        ...
        foreach ($loc['students'] as $student) {
            ...
            foreach ($student['exam_details'] as $examDetail) {
                ...
                if ($isFirstRow) {
                    $html       .= '<td rowspan="' . $examCount . '">' . toPersianDigits($studentIndex) . '</td>';
                    $html       .= '<td rowspan="' . $examCount . '">' . toPersianDigits($student['student_id']) . '</td>';
                    $html       .= '<td rowspan="' . $examCount . '" style="text-align: right;">' . $student['student_name'] . '</td>';
                    $isFirstRow  = false;
                }

                $html .= '<td style="text-align: right;">' . toPersianDigits($examDetail['course_code']) . ' - ' . $examDetail['course_name'] . '</td>';
                $html .= '<td>' . toPersianDigits($examDetail['seat_number']) . '</td>';
                $html .= '<td style="text-align: right; font-size: 7pt;">' . $examDetail['location'] . '</td>';

                if ($examDetail['is_primary']) {
                    $html .= '<td style="text-align: center;">همین مکان</td>';
                } else {
                    $html .= '<td style="text-align: right; font-size: 7pt;">' . $examDetail['send_to'] . '</td>';
                }
