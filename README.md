...
### تغییرات نسخه 2.2.4
- **بررسی کد ساد تکراری**: اضافه شدن validation برای کد لایسنس `00000000000000000000000000000000`
- اگر کد ساد قبلاً در سیستم دیگری ثبت شده باشد، پیام خطا نمایش داده می‌شود
- پس از نمایش خطا، مودال تنظیمات دوباره نمایش داده می‌شود تا کاربر بتواند کد صحیح را وارد کند
- بهبود تجربه کاربری در فرآیند راه‌اندازی اولیه
...
- اصلاح ارسال پارامترهای SaadCode و Center به سرور لایسنس
- اکنون درخواست لایسنس به درستی با query parameters ارسال می‌شود (GET request)
- رفع مشکل "سرور لایسنس پاسخ نامعتبر داد" در نصب‌های جدید

### تغییرات نسخه 2.2.1
- **رفع باگ**: اضافه شدن قابلیت درخواست خودکار لایسنس آزمایشی در اولین اجرا
- کاربرانی که سیستم را برای اولین بار راه‌اندازی می‌کنند دیگر با خطای "License token is missing" مواجه نمی‌شوند
- سیستم به طور خودکار با ارسال درخواست به سرور لایسنس، توکن موقت دریافت می‌کند
...
### ✨ قابلیت‌های جدید
- **گزارش‌های تعاملی داشبورد**: امکان کلیک روی کارت‌های آمار برای دریافت گزارش‌های تفصیلی
  - کارت دانشجویان: جستجو با شماره دانشجویی و نمایش آزمون‌های دانشجو
  - کارت آزمون‌ها: جستجو با کد درس و نمایش لیست دانشجویان درس
  - کارت آزمون بعدی: نمایش خودکار دانشجویان و دروس جلسه آزمون
- **کارت گزارش درخواستی**: نمایش یکپارچه گزارش‌ها در یک کارت اختصاصی با دکمه پاک‌سازی
- **جداول پیشرفته**: نمایش اطلاعات کامل با جزئیات استقرار، شماره صندلی و مشخصات

### 🎨 بهبود رابط کاربری
- حذف اسکرول بار از کارت گزارش‌ها (گسترش خودکار کارت)
- حذف اسکرول بار از input های SweetAlert
- نمایش تعداد شرکت‌کنندگان هر درس در لیست آزمون‌ها
- بهینه‌سازی جداول با badge های رنگی برای نوع آزمون

### 🔧 API های جدید
- `getStudentReport.php`: دریافت گزارش دانشجو با شماره دانشجویی
- `getCourseReport.php`: دریافت گزارش درس با کد درس  
- `getNextExamReport.php`: دریافت گزارش آزمون بعدی با تاریخ و ساعت
...
- **محافظت API**: حفاظت از تمام endpoints با License Guard
- **Audit Logging**: ثبت رویدادهای امنیتی
- **بهبود مدیریت خطا**: حذف error suppression و سخت‌تر کردن SSL verification

### � آپلود فایل دیتابیس
- **رابط کاربری مدرن**: پاپ‌آپ زیبا با Drag & Drop و Progress Bar
- **دکمه‌های آزمون**: آپلود جداگانه برای آزمون‌های کتبی (K.xlsx) و الکترونیکی (E.xlsx)
- **امنیت آپلود**: اعتبارسنجی فایل، MIME type و حجم (تا 128MB)
- **تشخیص خودکار محدودیت**: گرفتن حداکثر حجم از تنظیمات PHP

### 🎨 رابط کاربری
- دکمه‌های سبز با افکت‌های زیبا
- ناحیه Drag & Drop با انیمیشن
- دکمه فارسی "انتخاب فایل"
